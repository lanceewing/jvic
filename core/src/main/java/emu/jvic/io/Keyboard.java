...
public class Keyboard {
    ...
    /**
     * HashMap used to store mappings between Java key events and VIC 20 keyboard
     * scan codes.
     */
    ...
    /**
     * Holds the last time that the key was pressed down, or 0 if it has since been
     * released.
     */
    private long minKeyReleaseTimes[] = new long[512];

    /**
     * Holds a queue of keycodes whose key release processing has been delayed. This
     * is supported primarily for use with the Android virtual keyboard on some
     * devices, where the key pressed and release both get fired on release of the
     * key, so have virtually no time between them.
     */
    ...
    public void keyPressed(int keycode) {
        // Store the minimum expected release time for this key, i.e. current time +
        // 50ms.
    ...
    public void keyReleased(int keycode) {
        ...
        if (currentTime < minKeyReleaseTime) {
            ...
            // Otherwise we process the release by updating the key matrix that the VIC
            // polls.
            int keyDetails[] = (int[]) keyConvHashMap.get(new Integer(keycode));
            if (keyDetails != null) {
                keyMatrix[keyDetails[1]] &= (~keyDetails[2]);
    ...
    public int scanKeyboardRow(int selectedRow) {
        ...
        if ((selectedRow & 0x80) != 0)
            columnData |= keyMatrix[0x80];
        if ((selectedRow & 0x40) != 0)
            columnData |= keyMatrix[0x40];
        if ((selectedRow & 0x20) != 0)
            columnData |= keyMatrix[0x20];
        if ((selectedRow & 0x10) != 0)
            columnData |= keyMatrix[0x10];
        if ((selectedRow & 0x08) != 0)
            columnData |= keyMatrix[0x08];
        if ((selectedRow & 0x04) != 0)
            columnData |= keyMatrix[0x04];
        if ((selectedRow & 0x02) != 0)
            columnData |= keyMatrix[0x02];
        if ((selectedRow & 0x01) != 0)
            columnData |= keyMatrix[0x01];
    ...
    public int scanKeyboardColumn(int selectedColumn) {
        ...
        if ((selectedColumn & 0x80) != 0)
            rowData |= keyMatrix[0x80];
        if ((selectedColumn & 0x40) != 0)
            rowData |= keyMatrix[0x40];
        if ((selectedColumn & 0x20) != 0)
            rowData |= keyMatrix[0x20];
        if ((selectedColumn & 0x10) != 0)
            rowData |= keyMatrix[0x10];
        if ((selectedColumn & 0x08) != 0)
            rowData |= keyMatrix[0x08];
        if ((selectedColumn & 0x04) != 0)
            rowData |= keyMatrix[0x04];
        if ((selectedColumn & 0x02) != 0)
            rowData |= keyMatrix[0x02];
        if ((selectedColumn & 0x01) != 0)
            rowData |= keyMatrix[0x01];
