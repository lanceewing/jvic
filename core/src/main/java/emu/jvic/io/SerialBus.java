 ...
 * 
 * The serial bus lines are open collector. The individual devices can pull the
 * lines down to ground to indicate that the signal is TRUE (they use an open
 * collector inverter to do this), but if the signal should be FALSE, then they
 * let the line float. If no device is pulling a line down, then all devices
 * interpret that as a FALSE value for that line (they have internal pull ups,
 * and then some devices, like the 1541, then invert it to become a LOW signal,
 * but others don't have an inverter and instead read the pulled up HIGH; I
 * guess its whatever they preferred when designing).
 * 
 * -A line will become "true" (PULLED DOWN, or 0V) if one or more devices signal
 * true; -A line will become "false" (RELEASED, or 5V) only if all devices
 * signal false.
 * 
 * We emulate this behaviour by using a HashSet for each line that keeps track
 * of what devices (or caller to this class) are currently pulling each line
 * down. There can be multiple devices pulling a line down at the same time.
 * Only when a HashSet is empty will the associated line be considered floating.
 * 
 * @author Lance Ewing
 */
public class SerialBus {
    ...
    /**
     * Pull down HashSet for the ATN (attention) line. It is only the VIC 20 that
     * will pull this line down, but we treat it the same as the other lines for
     * consistency.
     * 
     * Sending any byte with the ATN line low (sending under attention) causes it to
     * be interpreted as a bus command for peripherals on the serial bus. When the
     * VIC 20 brings this signal LOW, all other devices start listening for it to
     * transmit an address. The device addressed must respond in a preset period of
     * time; otherwise, the VIC 20 will assume that the device addressed is not on
     * the bus.
     */
    ...
    // If true, then the record() method with store the history of any changes in
    // the lines.
    ...
    /**
     * If debug is true, then calling this method each cycle will store the history
     * of any changes to the serial lines in the wavedrom.com format. This is useful
     * for debugging visually what is happening on the serial bus.
     */
    public void record() {
        if (debug) {
