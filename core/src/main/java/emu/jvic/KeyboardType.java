...
public enum KeyboardType {
     ...
     */
    // TODO: Remove the multiple textures.
    ...
    public Integer getKeyCode(float x, float y, int side) {
        ...
        switch (this) {
        case LANDSCAPE:
            // Perform adjustment of x pos for rows 0, 1 and 4 to align with rows 2 and 3,
            // but
    ...
    public float getHeight() {
        if (isLandscape()) {
            ...
            return Math.max(Math.min(keyboardHeight, getTexture().getHeight()), 365);
        }
    }
    ...
    public boolean isPortrait() {
        return (equals(PORTRAIT) || equals(PORTRAIT_12x6));
    }
