...
/**
 * This class represents a PCVIC snapshot file. PCVIC was one of the early VIC
 * 20 emulators for DOS.
 * 
 * @author Lance Ewing
 */
public class PcvSnapshot extends Snapshot {
    ...
    private void decode(byte[] pcvData) {
        ...
        char[] signatureChars = new char[] { 
                (char) pcvData[0], (char) pcvData[1], (char) pcvData[2], (char) pcvData[3],
                (char) pcvData[4], (char) pcvData[5], (char) pcvData[6], (char) pcvData[7], 
                (char) pcvData[8], (char) pcvData[9], (char) pcvData[10], (char) pcvData[11], 
                (char) pcvData[12], (char) pcvData[13], (char) pcvData[14], (char) pcvData[15], 
                (char) pcvData[16], (char) pcvData[17], (char) pcvData[18], (char) pcvData[19], 
                (char) pcvData[20] };
     ...
     * 
     * @return The index within the raw data for the byte immediately after the
     *         compressed memory area.
     */
    private int uncompressMemory(byte[] pcvData, int pcvDataIndex, int startAddress, int endAddress) {
        int leaderByte, value, memoryIndex = startAddress, expectedBytes = ((endAddress - startAddress) + 1), 
                n, runningTotal = 0;
    ...
    /**
     * Decodes the PCVIC snapshot's Register State Block (RSB). This is the block
     * that contains the CPU registers, VIC registers and VIA chip registers that
     * aren't memory mapped.
     * 
     * @param pcvData The raw data of the PCVIC snapshot file.
     * 
     * @return The index within the raw data for the byte immediately after the
     *         Register State Block (RSB).
     */
    private int decodeRegisterStateBlock(byte[] pcvData) {
        ...
        // JVic assumes a high byte value of 0x01 and therefore doesn't store it in the
        // stackPointer value.
        ...
        ramExpansion = 0;
        if ((memoryConfig & 0x01) > 0)
            ramExpansion |= (Vic20Memory.RAM_1 | Vic20Memory.RAM_2 | Vic20Memory.RAM_3);
        if ((memoryConfig & 0x02) > 0)
            ramExpansion |= Vic20Memory.BLK_1;
        if ((memoryConfig & 0x04) > 0)
            ramExpansion |= Vic20Memory.BLK_2;
        if ((memoryConfig & 0x08) > 0)
            ramExpansion |= Vic20Memory.BLK_3;
        if ((memoryConfig & 0x20) > 0)
            ramExpansion |= Vic20Memory.BLK_5;
        ...
        // Aux flags (Bit2..Bit5 are Bit2..Bit5 (1,B,D,I) of P register. Others should be 0)
        processorStatusRegister = ((mainFlags & 0x0080) > 0 ? 0x80 : 0) | ((mainFlags & 0x4000) > 0 ? 0x40 : 0) |
        // TODO: Investigate why the B flag is set in some snapshots. Currently we're
        // clearing it to avoid immediate RESET.
                (auxFlags & 0x2C) | ((mainFlags & 0x0040) > 0 ? 0x02 : 0) | ((mainFlags & 0x0100) > 0 ? 0x01 : 0);

        // A register
        accumulator = ((int) pcvData[pcvDataIndex++] & 0xFF);

        // Scan count (CPU cycle count within scan line, 0 = End of scan line, 129(NTSC)
        // or 131(PAL) = Start of scan line).
